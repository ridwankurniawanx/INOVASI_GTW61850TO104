...
def command_60870_callback(ioa, ioa_data, iec104server, select_value):
    ...
    if not client:
        logger.error(f"No active client found for IED '{ied_id}' to handle command for IOA {ioa}")
        return -1
...
def ied_thread_worker(ied_id, uris_to_register, stop_event):
    ...
    try:
        ...
        client.getRegisteredIEDs()
        logger.info(f"[{ied_id}] Data points registered.")
        ...
        logger.error(f"An error occurred in thread for IED {ied_id}: {e}", exc_info=True)
    finally:
        # Hapus client dari kamus global saat thread berhenti
        with client_lock:
            if ied_id in ied_clients:
                del ied_clients[ied_id]
        logger.info(f"Thread for IED {ied_id} has stopped.")


if __name__ == '__main__':
    logger = logging.getLogger('gateway')
    logging.basicConfig(format='%(asctime)s [%(threadName)-10s] %(name)-12s %(levelname)-8s %(message)s',
                        level=logging.INFO)
    ...
    logger.info("Gateway v3.1 (Multi-Threaded) started")
