#!/usr/bin/env python3
# gateway_v5.9.py - Final Robust Version
# Deskripsi: Gateway IEC 61850 ke IEC 60870-5-104 yang stabil.
# Fitur v5.9: Inisialisasi semua objek asyncio (Queue dan Event) di dalam
#             main() untuk menjamin keamanan thread dan loop.
# v5.9 Khusus untuk Rockylinux
...
# Sebaiknya, library diimpor di sini. Jika ada yang spesifik, pastikan sudah terinstal.
...
# --- Variabel & Objek Global (Hanya yang Aman untuk Thread) ---
...
def invalidate_ied_points(ied_id, queue):
    try:
        queue.put_nowait({'type': 'invalidate', 'ied_id': ied_id})
    except asyncio.QueueFull:
        logging.warning(f"Update queue is full, invalidation request for {ied_id} was dropped.")

# --- ASYNC TASKS ---

async def ied_handler(ied_id, uris, queue, shutdown_event):
    ...
    def ied_specific_callback(key, data):
        try:
            queue.put_nowait({'type': 'data', 'ied_id': ied_id, 'key': key, 'data': data})
        except asyncio.QueueFull:
    ...
    while not shutdown_event.is_set():
        try:
            ...
            invalidate_ied_points(ied_id, queue)
...
async def iec104_processor(queue, shutdown_event):
...
async def main():
    ...
    logger = logging.getLogger('gateway-async-final')
    ...
    logger.info("Gateway v5.9 (Asyncio, Final Robust Mode) started")
    ...
    # Membuat semua objek asyncio di dalam lingkup async utama.
    update_queue = asyncio.Queue()
    shutdown_event = asyncio.Event()

    # Meneruskan semua objek yang dibutuhkan sebagai argumen.
    tasks = [ied_handler(ied_id, uris, update_queue, shutdown_event) for ied_id, uris in ied_data_groups.items()]
    tasks.append(iec104_processor(update_queue, shutdown_event))
