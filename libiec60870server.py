...
class IEC60870_5_104_server:
    ...
    def update_ioa(self, ioa, data):
        ...
        # ================================================================= #
        value = 0  # Nilai default
        io_type = self.IOA_list[ioa]['type']

        if io_type == MeasuredValueShort:
            value = float(data)
        elif io_type == DoublePointInformation:
            # Konversi spesifik untuk Double Point Information
            # IEC 61850 mengirimkan 1 (OFF) atau 2 (ON)
            # IEC 104 juga mengharapkan 1 (OFF) atau 2 (ON)
            # Namun, jika sumbernya boolean (true/false -> 1.0/0.0), kita perlu konversi
            float_data = float(data)
            if float_data == 0.0: # Jika sumbernya boolean FALSE
                value = 1  # State OFF untuk IEC 104
            elif float_data == 1.0: # Jika sumbernya boolean TRUE atau Dbpos OFF
                value = 1  # State OFF untuk IEC 104
            elif float_data == 2.0: # Jika sumbernya Dbpos ON
                value = 2  # State ON untuk IEC 104
            else:
                value = int(float_data) # Fallback
        else:
            # Untuk tipe data lain seperti SinglePoint atau ScaledValue
...
if __name__== "__main__":
    ...
    # Contoh penggunaan dengan tipe data baru
    ...
    while True:
        time.sleep(2)
        srv.update_ioa(100, val + 0.1) # Update nilai float
        srv.update_ioa(101, int(val)) # Update nilai scaled
        val = val + 1.0
